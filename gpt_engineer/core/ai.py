...
Classes:
    ...
    GeminiAI: Enhanced AI class using Google Gemini models with multimodal capabilities.
...
class AI:
    ...
    def __init__(
        ...
        self.vision = (
            ...
            or ("gemini" in model_name and ("vision" in model_name or "1.5" in model_name))
        )
    ...
    def _create_chat_model(self) -> BaseChatModel:
        ...
        if self.azure_endpoint:
            ...
            model_type = "multimodal" if "2.0" in self.model_name else "standard"
            
            return create_gemini_ai(
                model_type=model_type,
                model_name=self.model_name,
                temperature=self.temperature,
                streaming=self.streaming,
                enable_image_generation=os.getenv("ENABLE_GEMINI_IMAGE_GENERATION", "true").lower() == "true",
                enable_video_generation=os.getenv("ENABLE_GEMINI_VIDEO_GENERATION", "true").lower() == "true",
                enable_multimodal=os.getenv("ENABLE_GEMINI_MULTIMODAL", "true").lower() == "true",
            )
        elif self.vision:
            return ChatOpenAI(
                model=self.model_name,
