...
def main(
    ...
    model: str = typer.Option(
        os.environ.get("MODEL_NAME", "gemini-2.0-flash"), "--model", "-m", help="model id string"
    ),
    ...
    generate_assets: bool = typer.Option(
        False,
        "--generate-assets",
        help="Generate visual assets (diagrams, mockups, videos) using Gemini capabilities.",
    ),
    gemini_multimodal: bool = typer.Option(
        False,
        "--gemini-multimodal",
        help="Use Gemini's advanced multimodal capabilities for enhanced generation.",
    ),
    create_tutorials: bool = typer.Option(
        False,
        "--create-tutorials",
        help="Generate interactive tutorials and video content using Gemini.",
    ),
):
    """
    ...
    if llm_via_clipboard:
        ...
        from gpt_engineer.core.gemini_ai import create_gemini_ai
        
        model_type = "multimodal" if gemini_multimodal or "2.0" in model else "standard"
        ai = create_gemini_ai(
            model_type=model_type,
            model_name=model,
            temperature=temperature,
            enable_image_generation=generate_assets,
            enable_video_generation=create_tutorials,
            enable_multimodal=gemini_multimodal or generate_assets or create_tutorials,
        )
    else:
        ai = AI(
            model_name=model,
    ...
    elif "gemini" in model.lower() and (generate_assets or gemini_multimodal):
        # Use enhanced Gemini generation with visual assets
        from gpt_engineer.tools.gemini_steps import gemini_gen_code
        code_gen_fn = lambda ai, prompt, memory, preprompts_holder: gemini_gen_code(
            ai, prompt, memory, preprompts_holder, generate_assets=generate_assets
        )
    else:
    ...
    if not no_execution:
        if improve_mode:
            ...
            # Generate additional Gemini-specific content
            if "gemini" in model.lower() and not no_execution:
                from gpt_engineer.tools.gemini_steps import (
                    gemini_gen_visual_assets,
                    gemini_generate_documentation,
                    gemini_create_tutorials
                )
                from gpt_engineer.core.gemini_ai import GeminiMultimodalAI
                
                # Generate visual assets if requested
                if generate_assets and isinstance(ai, GeminiMultimodalAI):
                    print(colored("Generating visual assets...", "blue"))
                    assets = gemini_gen_visual_assets(ai, files_dict, prompt, memory)
                    if assets:
                ...
                if generate_assets or gemini_multimodal:
                    print(colored("Generating enhanced documentation...", "blue"))
                    docs = gemini_generate_documentation(
                        ai, files_dict, prompt, memory, include_visuals=generate_assets
                    )
                    files_dict.update(docs)
                    print(f"Generated {len(docs)} documentation files")
                
                # Create tutorials if requested
                if create_tutorials and isinstance(ai, GeminiMultimodalAI):
                    print(colored("Creating interactive tutorials...", "blue"))
                    tutorials = gemini_create_tutorials(ai, files_dict, prompt, memory)
                    if tutorials:
                        print(f"Created {len(tutorials)} tutorial resources")
                        for tutorial_name in tutorials.keys():
